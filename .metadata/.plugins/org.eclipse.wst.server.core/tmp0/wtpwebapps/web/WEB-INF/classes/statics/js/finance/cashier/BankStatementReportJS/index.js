new Vue({
    el: '#bank-statement-report',//银行对账日报表
    data() {
        return {
            openTime: '',
            ...
            base_config: {
                ...
                url: contextPath + '/bankStatement/getBankStatement',
                datatype: 'json',
                jsonReader: {
                    root: "data",
                    cell: "data",
    ...
    methods:{
        ...
        jqGridInit(colNames,colModel,headers) {
            ...
            let config = Object.assign({}, _vm.base_config, {
                ...
                postData: {"queryDate":_vm.formData.queryDate,
                    ...
                    if (ret !=null && ret.data != null) {
                        for (var index in ret.data) {
                            if (ret.data[index].yesterdayBalanceFor != null && ret.data[index].yesterdayBalanceFor != 0) {
                                ret.data[index].yesterdayBalanceFor = ret.data[index].yesterdayBalanceFor.toFixed(2);
                            }

                            if (ret.data[index].yesterdayBalance != null && ret.data[index].yesterdayBalance != 0) {
                                ret.data[index].yesterdayBalance = ret.data[index].yesterdayBalance.toFixed(2);
                            }

                            if (ret.data[index].debitAmountFor != null && ret.data[index].debitAmountFor != 0) {
                                ret.data[index].debitAmountFor = ret.data[index].debitAmountFor.toFixed(2);
                            }

                            if (ret.data[index].debitAmount != null && ret.data[index].debitAmount != 0) {
                                ret.data[index].debitAmount = ret.data[index].debitAmount.toFixed(2);
                            }

                            if (ret.data[index].creditAmountFor != null && ret.data[index].creditAmountFor != 0) {
                                ret.data[index].creditAmountFor = ret.data[index].creditAmountFor.toFixed(2);
                            }

                            if (ret.data[index].creditAmount != null && ret.data[index].creditAmount != 0) {
                                ret.data[index].creditAmount = ret.data[index].creditAmount.toFixed(2);
                            }

                            if (ret.data[index].todayBalanceFor != null && ret.data[index].todayBalanceFor != 0) {
                                ret.data[index].todayBalanceFor = ret.data[index].todayBalanceFor.toFixed(2);
                            }

                            if (ret.data[index].todayBalance != null && ret.data[index].todayBalance != 0) {
                                ret.data[index].todayBalance = ret.data[index].todayBalance.toFixed(2);
                            }
                        }
                    }
        ...
        exportData() {
            let _that = this;
            window.open(contextPath + "/bankStatement/exportBankStatement?queryDate=" + _that.formData.queryDate + "&displayCurrencyType=" + _that.displayCurrencyType + "&isDisplayDisabledSubject=" + _that.formData.isDisplayDisabledSubject + "&isDisplayDetail=" + _that.formData.isDisplayDetail + "&isDisplayCount=" + _that.formData.isDisplayCount + "&isDisplayTotal=" + _that.formData.isDisplayTotal);
        },
        print() {
            let that = this;
            ...
            if (that.displayCurrencyType == '1') {
                ...
                var _info = {
                    ...
                    'colNames': [       // 列名与对应字段名
                        ...
                        { 'name': '昨日余额', 'col': 'yesterdayBalanceFor', 'sum': true},
                        { 'name': '今日借方', 'col': 'debitAmountFor', 'sum': true},
                        { 'name': '今日贷方', 'col': 'creditAmountFor', 'sum': true},
                        { 'name': '今日余额', 'col': 'todayBalanceFor', 'sum': true}
                    ],
                ...
                var _info = {
                    ...
                    'colNames': [       // 列名与对应字段名
                        ...
                        { 'name': '银行名称', 'col': 'bankName' },
                        { 'name': '银行账号', 'col': 'bankAccount' },
                        { 'name': '昨日余额', 'col': 'yesterdayBalance', 'sum': true},
                        { 'name': '今日借方', 'col': 'debitAmount', 'sum': true},
                        { 'name': '今日贷方', 'col': 'creditAmount', 'sum': true},
                        { 'name': '今日余额', 'col': 'todayBalance', 'sum': true}
                    ],
                    'styleCss': '',   // 自定义样式 例：  thead{text-align: center;font-size: 14px;font-weight: 100;}
                    'colMaxLenght': 9,  // 显示最大长度， 默认为7
                    'totalRow': false,
