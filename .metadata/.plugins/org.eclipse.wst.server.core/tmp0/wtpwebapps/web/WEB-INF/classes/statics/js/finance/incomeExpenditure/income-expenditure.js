var ve = new Vue({
    ...
    methods: {
        ...
        subjectData(treeNode) {
            ...
            this.body.subjectId = treeNode.id;
            this.body.subjectCode = treeNode.subjectCode;
            this.body.subjectName = treeNode.subjectName;
            this.subjectClose();
        ...
        //点击修改
        modify() {
            let dateSize = this.selected.length;
            //设置为更新操作
            console.log(this.selected);
            if (dateSize > 1) {
                layer.alert('修改只能对单条数据进行操作');
            } else if (dateSize <= 0) {
                layer.alert('请选择需要修改操作的数据');
            } else {
                ...
                $.ajax({
                    ...
                    async: true,
                    data: {id: id},
                    url: contextPath + '/incomeCategory/queryListPage',
                    dataType: 'json',
                    success: function (d) {
        ...
        deleteBatch1: function () {
            ...
            $.ajax({
                url: contextPath + '/incomeCategory/deleteBatch',
                method: 'post',
                dataType: "json",
        ...
        saveData: function () {
            ...
            if (ve.isSaveOrUpdata) {
                $.ajax({
                    ...
                    url: contextPath + '/incomeCategory/update',
                    dataType: 'text',
                    success: function (d) {
                        that.isSaveOrUpdata = false;
                        var data = eval('(' + d + ')');
                        if ("codeNo" == data.data) {
                ...
                $.ajax({
                    ...
                    contentType: 'application/text',
                    data: JSON.stringify(this.body),
                    url: contextPath + '/incomeCategory/save',
                    dataType: 'text',
                    success: function (d) {
                        var data = eval('(' + d + ')');
                        if ("codeNo" == data.data) {
    ...
    mounted() {
        ...
        $.ajax({
            ...
            async: true,
            data: '',
            url: contextPath + '/incomeCategory/getAccountSubjectList',
            dataType: 'json',
            success: function (d) {
                var list = d.data;
                var datas = [];
