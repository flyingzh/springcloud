new Vue({
    ...
    methods:{
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (data!= null) {
                        ...
                        that.sDate = that.formData.date1;
                        var length = data.monthList.length;
                        if(length>0){
                            that.eDate = that.getLastDay(data.currentYear,data.monthList[length-1].value);  //结束日期;
                        }
                    }
                }
            });
        ...
        titleParams(){
            var currencyType = this.formData.currencyType;  //币别类型
        ...
        queryParams(){  //查询参数
            ...
            if(dateType=='2'){
                if(this.formData.date1<new Date(this.sDate.replace(/-/g,   "/"))|| this.formData.date2>new Date(this.eDate.replace(/-/g,   "/"))){
                    return false;
                }
        ...
        setTableHeader() {
            ...
            if(currencyType=='3') {  //原币和本位币
                this.colNames = ['id', '科目代码', '科目名称', '币别', '银行名称', '银行账号', '原币', '本位币', '原币', '本位币', '原币', '本位币', '原币', '本位币','借方笔数','贷方笔数','启用','科目类别'];
                this.colModel = [
                    {name: 'id', width: 30, hidden:true,frozen:true},
                    {name: 'accountCode', width:100,frozen:true},
                    {name: 'accountName', width:150,frozen:true},
                ...
                this.tableHeaders = [
                    ...
                    { startColumnName: 'debitAmountFor', numberOfColumns: 2, titleText: '借方发生额'},
                    { startColumnName: 'creditAmountFor', numberOfColumns: 2, titleText: '贷方发生额'},
        ...
        save() {
            if(!this.queryParams()){
                this.$Modal.info({
                    title: '信息提示',
                    content: '有效日期范围是:'+this.operateDate(this.sDate)+' 至 '+this.operateDate(this.eDate)
                });
                return;
            }
