...
var report=new Vue({
    ...
    methods: {
        ...
        init() {
            ...
            $.ajax({
                url: contextPath+'/testReportDocumentController/genReportDocWithoutSD',
                type: 'post',
                dataType: "json",
        ...
        approvalInfoInit(){
            ...
            $.ajax({
                ...
                type: 'post',
                dataType: "json",
                data: {reportDocCode:_this.all.reportDocCode},
        ...
        reject() {
            ...
            This.reportDocument.testDocumentId=This.all.testDocumentId;
            This.reportDocument.reportDocCode=This.all.reportDocCode;
            This.initApproval(This.all.reportDocCode);
                //发送请求
                $.ajax({
                    type: "POST",
                    url: contextPath+"/testReportDocumentController/findUserOperation",
                        ...
                        if(data.code === "100514"){
                            let msg = data.data === 1 ? "【一级审核】":data.data === 2 ?
                                "【二级审核】":data.data === 3 ? "【三级审核】":data.data === 4 ?
                                    "【四级审核】":data.data === 5 ? "【五级审核】":data.data === 6 ?
                                        "【六级审核】":data.msg;
                            alertMsg(data.data === 0 ? "该单据已被驳回到申请人，待申请人提交!":"您没有"+msg+"的驳回权限",2);
                        }
                        if(data.code === "100515"){
                            alertMsg("驳回成功");
        ...
        initApproval(){
            ...
            $.ajax({
                ...
                data:{receiptsId:This.all.reportDocCode},
                dataType: "json",
                success: function (data) {
    ...
    watch:{
        "all.documentStatus":function () {
            if(this.all.documentStatus && this.all.documentStatus === '已审核'){
